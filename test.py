...
test_dataset, test_data_loader = val_loader('images/test/distorted', 'images/test/test.csv', shuffle=False, batch_size=1, num_workers=4)

experiment = 'exp_1'
os.makedirs(f'images/test/rectified/{experiment}', exist_ok=True)

model = torch.load(f'models/{experiment}.pth').cuda()
...
with tqdm(test_data_loader, leave=False) as test_loop:
    with torch.no_grad():
        for i, (feature, k) in enumerate(test_loop):
            ...
            results.append(image_file, k.item())

            image = cv2.imread(f'images/test/distorted/{image_file}')
            image = transform.rectify(image, k.item())
